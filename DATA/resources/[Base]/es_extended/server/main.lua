...
function onPlayerJoined(playerId)
  ...
  if identifier then
    if ESX.GetPlayerFromIdentifier(identifier) then
      DropPlayer(playerId,
        ('Gặp lỗi khi tải nhân vật của bạn!\n\nCó người đang chơi tài khoản của bạn trong server.\n\nSteam: %s'):format(
          identifier))
    else
    ...
    DropPlayer(playerId,
      'Gặp lỗi khi tải nhân vật của bạn!\nError code: identifier-missing-ingame\n\nVui lòng đăng nhập steam để trải nghiệm server!')
  end
end
...
if not Config.Multichar then
  AddEventHandler('playerConnecting', function(name, setCallback, deferrals)
    ...
    if identifier then
      if ESX.GetPlayerFromIdentifier(identifier) then
        deferrals.done(
          ('[ESX] Gặp lỗi khi tải nhân vật của bạn!\n\nLỗi này xảy ra do một người chơi trên máy chủ này có cùng mã định danh với bạn. Đảm bảo bạn không chơi trên cùng một tài khoản.\n\nYour identifier: %s'):format(
            identifier))
      else
        deferrals.done()
      end
