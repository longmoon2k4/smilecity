...
GetVehicles = function(source,house)
  local vehs = {}
  local retData = SqlFetch("SELECT plate, vehicle FROM owned_vehicles WHERE storedhouse=@storedhouse",{['@storedhouse'] = house.Id})
...
GetInventory = function(source,cb,house)
  ...
  cb({cannangkhodo = v.CanNangKhoDo, cash = v.Inventory.Cash, blackMoney = v.Inventory.DirtyMoney, items = v.Inventory.Items, weapons = v.Inventory.Weapons })
end
  ...
  if not Config.RestrictHouseCount or owned_count < Config.RestrictHouseCount then
    ...
    if not v.Owned then
      ...
      if afford then
        ...
        TriggerClientEvent("esx:showNotification",_source,"Bạn đã mua nhà với giá $"..v.Price..".")
        sendDiscord('Nhà', '***'..GetPlayerName(_source).. '*** đã mua nhà số '..v.Id..' với giá '..v.Price)
      --  TriggerEvent("DiscordBot:LogDiscord","https://discord.com/api/webhooks/tatwebhooks/1034342797836296212/RidsF-sTXmtDjh-pUjXFypmdoSvcN897F8NQHpBmvLRvWS7YvMt4osOWP-oMIRr41HeX", '***'..GetPlayerName(_source).. '*** đã mua nhà số '..v.Id..' với giá '..v.Price)
      else
        TriggerClientEvent("esx:showNotification",_source,"You can't afford that.")
      end
    end
  else
    TriggerClientEvent("esx:showNotification",_source,"Bạn đã có quá nhiều nhà.")
  end
end
...
SqlCheck = function()
  ...
  if curInDb and type(curInDb) == "table" then
    for k,v in pairs(curInDb) do
      ...
      exports.ox_inventory:RegisterStash('house'..v.id, 'Kho nhà '..v.id, 100, v.cannangkhodo, false)
    end
  end
  ...
  if Houses and type(Houses) == "table" then
    for k1,v1 in pairs(Houses) do
      ...
      if not foundMatch then
        ...
        newHouses[HouseKey].CanNangKhoDo = 5000000
        ...
        newHouses[curInDb[foundMatch].id].CanNangKhoDo = (curInDb[foundMatch].cannangkhodo or 5000000)
      end
    end
  end
  if curInDb and type(curInDb) == "table" then
...
CreateHouse = function(house)
  ...
  local _key = HouseKey
  HouseKey = HouseKey + 1
  exports.ox_inventory:RegisterStash('house'.._key, 'Nhà '.._key, 1000, 2000000, false)
  local charName = GetCharacterName(_source)
  Houses[_key] = {
    Id = _key,
    Owner = identifier,
...
SetWardrobe = function(house,wardrobe)
  ...
  if v.Owned == true and v.Owner == GetPlayerIdentifier(_source) then
    v.Wardrobe = wardrobe
    SyncHouse(v)
    SqlExecute("UPDATE "..((Config and Config.AllhousingTable) or "allhousing").." SET wardrobe=@wardrobe WHERE id=@id",{['@wardrobe'] = json.encode({x = wardrobe.x, 
y = wardrobe.y, z = wardrobe.z}),['@id'] = v.Id})
...
PutItem = function(house,itemType,itemName,itemCount)
  ...
  elseif itemType == 'item_standard' then
    ...
    local curWeight = 0;
    for _,item in pairs(v.Inventory.Items) do
      curWeight = curWeight + (item.count * item.weight)
    end
    local additemData = GetPlayerItemData(_source,itemName)
    local addWeight = additemData.weight*itemCount
    --print('addWeight',addWeight)
    totalWeight = curWeight + addWeight
    --print('totalWeight',totalWeight)
    if totalWeight <= v.CanNangKhoDo then
      ...
      NotifyPlayer(_source,"Kho nhà đã đầy.")
      return
    end
  elseif itemType == "item_weapon" then
    local playerWeapon,playerAmmo,weaponLabel,components = GetPlayerWeapon(_source,itemName)
...
GetCanNangKhoDo = function(source,house_id)
  if not house_id or not Houses or not Houses[house_id] then return; end
  return Houses[house_id].CanNangKhoDo
end
...
RegisterNetEvent("allhousing:nangcap")
AddEventHandler("allhousing:nangcap", function(id,money)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer.getMoney() >= money then
      MySQL.Async.execute("UPDATE allhousing SET cannangkhodo=cannangkhodo+1000 WHERE owner=@owner and id=@id",{['@owner'] = xPlayer.identifier,['@id'] = id})
      xPlayer.removeMoney(money)
      Houses[id].CanNangKhoDo = Houses[id].CanNangKhoDo + 500000
      TriggerClientEvent('esx:showNotification',src,"Nâng cấp thành công")
      --TriggerEvent("DiscordBot:LogDiscord","https://discord.com/api/webhooks/tatwebhooks/1044902996288483359/bv92l7skVXbKcpaRiFOgUSCZGcxhSP4qbCmJJ62ix3Dik7AHRSv9DpERixSZIgP94Znl", '***'..GetPlayerName(src).. '*** đã nâng cấp nhà '..id..'với giá 3m')
      sendDiscord("Nhà",'***'..GetPlayerName(src).. '*** đã nâng cấp nhà '..id..'với giá '.. money..'')
    else
      TriggerClientEvent('esx:showNotification',src,"Bạn không đủ tiền")
    end
end)
local webhookurl = "https://discord.com/api/webhooks/tatwebhooks/1052060938473308231/Ufrq00GlmJjBs1ARLpXmVnlY1pysjVjHwUFdxkVG8LZpdV_-6kgnNYFBiiX3J5hvRooQ"
