      ...
      if Config.Use3DText then
        if closestExtDist < Config.TextDistance3D then
          ...
          if closestExtDist < Config.InteractDistance then
            DrawText3D(house[closestExt].x,house[closestExt].y,house[closestExt].z, Labels["InteractDrawText"]..Labels[closestExt].." Nhà "..house.Id)
          else
      ...
      if closestExtDist < Config.InteractDistance then
        ...
        if IsControlJustReleased(0,Config.Controls.Interact) then
          if GetFakeWantedLevel() <= 0 then
            ...
            TriggerEvent('esx:showNotification',"Bạn đang bị truy nã không thể vào nhà!")
          end
        end
      end
