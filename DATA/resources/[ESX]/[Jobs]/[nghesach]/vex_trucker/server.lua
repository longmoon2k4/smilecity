...
AddEventHandler("truck_logistics:buyTruck",function(data)
	local source = source
	local user_id = ESX.GetPlayerFromId(source).identifier
	local username= GetPlayerName(source) .. user_id
	if user_id then
		if tryGetTruckerMoney(user_id,tonumber(data.price)) then
			...
			SendWebhookMessage(Config.webhook,Lang[Config.lang]['logs_buytruck']:format(data.truck_name,data.price,username..os.date("\n["..Lang[Config.lang]['logs_date'].."]: %d/%m/%Y ["..Lang[Config.lang]['logs_hour'].."]: %H:%M:%S")))
			openUI(source,true)
		else
			TriggerClientEvent("Notify",source,"negado",Lang[Config.lang]['insufficiente_funds'])
		end
	...
	if debug_cooldown[source] == nil then
		...
		if user_id then
			...
			if query then 
				...
				SendWebhookMessage(Config.webhook,Lang[Config.lang]['logs_selltruck']:format(data.truck_name,amount,username..os.date("\n["..Lang[Config.lang]['logs_date'].."]: %d/%m/%Y ["..Lang[Config.lang]['logs_hour'].."]: %H:%M:%S")))
				openUI(source,true)
			end
		end
		debug_cooldown[source] = nil
		...
		if user_id then
			...
			if not query or not query.remaining_amount or query.remaining_amount <= 0 then
				...
				if amount and amount > 0 then
					...
					openUI(source,true)
					SendWebhookMessage(Config.webhook,GetPlayerName(source)..user_id.." đã rút "..amount.."$")
				else
	...
	if user_id then
		...
		if amount and amount > 0 then
			...
			if money >= amount then
				...
				SendWebhookMessage(Config.webhook,GetPlayerName(source)..user_id.." đã gửi "..amount.."$")
				openUI(source,true)
			else
				TriggerClientEvent("Notify",source,"negado",Lang[Config.lang]['insufficiente_money'])
			end
...
AddEventHandler("truck_logistics:finishJob",function(data,distance,reward,truck_data,truck_engine,truck_body,trailer_body)
	...
	if user_id then
		...
		if level2 - level > 0 then
			...
			SendWebhookMessage(Config.webhook,Lang[Config.lang]['logs_skill']:format((level2 - level),username..os.date("\n["..Lang[Config.lang]['logs_date'].."]: %d/%m/%Y ["..Lang[Config.lang]['logs_hour'].."]: %H:%M:%S")))
		end
		SendWebhookMessage(Config.webhook,Lang[Config.lang]['logs_finish']:format(tostring(received_amount),tostring(received_xp),username..os.date("\n["..Lang[Config.lang]['logs_date'].."]: %d/%m/%Y ["..Lang[Config.lang]['logs_hour'].."]: %H:%M:%S")))
	end
end)
