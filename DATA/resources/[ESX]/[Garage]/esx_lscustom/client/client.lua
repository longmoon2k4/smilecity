...
local garages = { 
	...
	[7]= { locked = false, camera = {x = -215.518, y = -1329.135, z = 30.89, heading = 329.092}, driveout = {x = -205.935,y = -1316.642, z = 30.176, heading = 356.495}, drivein = {x = -205.626,y = -1314.99, z = 30.247, heading = 179.395}, outside = {x = -1648.457153, y=-3148.879150, z=13.980225, heading=325.984253}, inside = {x = -1651.463745, y=-3154.707764, z=13.980225, heading =5.669291} }
}
...
function refreshextra()
	for id,enabled in pairs(myveh.extra) do
		if enabled then
			SetVehicleExtra(myveh.vehicle, tonumber(id), 0)
		else
			SetVehicleExtra(myveh.vehicle, tonumber(id), 1)
		end
	end
end
	...
	if DoesEntityExist(veh) then
		...
		elseif type(LSC_Config.menu.position) == 'string' then
			...
			elseif  LSC_Config.menu.position == "right" then
				LSCMenu.config.position = { x = 1-0.16, y = 0.4}
			end
		end
		...
		local maxvehhp = 1000
		local damage = 0
		damage = (maxvehhp - GetVehicleEngineHealth(veh))/100
		LSCMenu:addPurchase("Repair vehicle",round(2500+500*damage,0), "Full body repair and engine service.")
		...
		AddMod(14,LSCMenu.categories,"HORN", "Horn", "Beep beep.",true)
		...
		LSCMenu.categories:addSubMenu("EXTRA", "Extra","Extra.", true)
		for id = 0, 12 do
			if DoesExtraExist(veh, id) then
				local btn = LSCMenu.categories.Extra:addPurchase("Extra " .. id,1000)
				btn.extraindex = id
			end
		end
	...
	while true do
		...
		if inside == false then
			...
			if IsPedSittingInAnyVehicle(ped) then
				...
				if DoesEntityExist(veh) and GetPedInVehicleSeat(veh, -1) == ped and (IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh)) or IsThisModelAHeli(GetEntityModel(veh)) or IsThisModelAPlane(GetEntityModel(veh)))  then
						...
						if LSC_Config.oldenter then			
							...
							if GetDistanceBetweenCoords(outside.x,outside.y,outside.z,GetEntityCoords(ped)) <= f(5) then
								...
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									...
									if pos.locked then
										...
										if not LSC_Config.lock then
											if IsControlJustPressed(1,201) then
												if IsVehicleDamaged(veh) then
													ESX.ShowNotification('Không thể độ khi xe bạn đang bị hỏng')
												else
													inside = true
													currentpos = pos
													currentgarage = i
												...
												drawTxt("Ấn ~b~[ENTER]~w~ để độ xe",4,1,0.5,0.8,1.0,255,255,255,255)
												DrawMarker(20, outside.x,outside.y,outside.z, 0, 0, 0, 0, 0, 90, 1.0, 1.0, 1.0, 0, 153, 255, 150, true, true, 2, true, false, false, false)
											end
										else
											drawTxt("~r~Đang có người sử dụng, chờ tí",4,1,0.5,0.8,1.0,255,255,255,255)
										end
									else
										if IsControlJustPressed(1,201) then
											if IsVehicleDamaged(veh) then
									...
									drawTxt("~r~Xe này không độ được",4,1,0.5,0.8,1.0,255,255,255,255)
								end
							end
						else
...
function LSCMenu:onSelectedIndexChanged(name, button)
	...
	elseif m == "extra" then
		refreshextra()
		-- local state = IsVehicleExtraTurnedOn(veh, button.extraindex)
		SetVehicleExtra(veh, tonumber(button.extraindex), 0)
...
AddEventHandler("LSC:buttonSelected", function(name, button, canpurchase)
	...
	elseif mname == 'extra' then
		if button.purchased or CanPurchase(price, canpurchase) then
			refreshextra()
			if IsVehicleExtraTurnedOn(veh, button.extraindex) == 1 then
				myveh.extra[tostring(button.extraindex)] = false
				SetVehicleExtra(veh, button.extraindex, 1)
			else
				myveh.extra[tostring(button.extraindex)] = true
				SetVehicleExtra(veh, button.extraindex, 0)
			end
		end
...
function CheckPurchases(m)
	...
	elseif name == 'extra' then
		for i,b in pairs(m.buttons) do
			if not myveh.extra[tostring(b.extraindex)] or myveh.extra[tostring(b.extraindex)] == 0 then
				b.sprite = nil
			else
				b.sprite = "garage"
			end
...
function CanPurchase(price, canpurchase)
	if canpurchase then
		if LSCMenu.currentmenu == "main" then
			ESX.ShowNotification("Phương tiện của bạn đã ~g~được sửa~w~.")
		else
			ESX.ShowNotification("Đã mua ~g~thành công~w~.")
		end
		return true
	else
		ESX.ShowNotification("Bạn ~r~không có đủ tiền~w~ để mua.")
		return false
	end
end
...
local function AddBlips()
	for i,pos in ipairs(garages) do
		...
		AddTextComponentString('<FONT FACE="arial font">Tiệm độ xe')
