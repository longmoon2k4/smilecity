...
RegisterKeyMapping('cancel', 'Huy hành động', 'keyboard', "x")
RegisterCommand('cancel', function()
  EmoteCancel() 
end, true)

RegisterKeyMapping('emotemenu', 'Menu Hành động', 'keyboard', "f3")
...
function EmoteCommandStart(source, args, raw)
  ...
  if delay > 0  then 
   -- TriggerEvent('esx:showNotification','Bạn phải đợi '..delay..'s nữa để có thể dùng hành động tiếp')
    return 
  else
    delay = 5
  end
  if (IsPedInAnyVehicle(PlayerPedId(),true)) then
    TriggerEvent('esx:showNotification','Bạn không thể dùng hành động trên xe')
    return 
  end
  if GetEntityHealth(PlayerPedId()) == 101 then
    return
