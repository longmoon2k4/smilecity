...
function MEMBER:EventHandler()
    RegisterNetEvent('sulu_member:client:open', function()
        self:Open()
    end)
    ...
    RegisterNUICallback("setJob", function(data, cb)
        ESX.TriggerServerCallback("sulu_member:server:setJob", function(callback) 
            if callback then
                self:Open()
            end
            cb(callback)
    ...
    RegisterNUICallback("Invite", function(data, cb)
        ESX.TriggerServerCallback("sulu_member:server:Invite", function(callback) 
            if callback then
                self:Open()
            end
            cb(callback)
...
function MEMBER:Open()
   -- print(ESX.DumpTable(self.PlayerData.job.name))
    ESX.TriggerServerCallback("sulu_member:server:getData",function(data, jobGrade, job)
