...
function Core.SavePlayer(xPlayer, cb)
  local parameters <const> = {
    ...
    GetPlayerName(xPlayer.playerId),
    xPlayer.identifier
  }

  MySQL.prepare(
    'UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?,`job2`= ?,`job2_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `name` = ? WHERE `identifier` = ?',
    parameters,
...
function Core.SavePlayers(cb)
  ...
  MySQL.prepare(
    "UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `name` = ? WHERE `identifier` = ?",
    parameters, 
    function(results)
      if not results then
        return
