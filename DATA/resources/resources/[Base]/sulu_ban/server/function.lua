				...
				if duree and duree < 365 then
					...
					local sourceplayername = ""
                        ...
                        for i=0, GetNumPlayerTokens(target), 1 do
                            table.insert(hwid,GetPlayerToken(target,i))
                        end
					if duree > 0 then
						ban(source,license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,duree,reason,0,hwid) --Timed ban here
						DropPlayer(target, Text.yourban .. reason)
					else
						ban(source,license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,duree,reason,1,hwid) --Perm ban here
					...
					if data[1] then
						if duree and duree < 365 then
							...
							if duree > 0 then --Here if not perm ban
								ban(source,data[1].license,data[1].identifier,data[1].liveid,data[1].xblid,data[1].discord,data[1].playerip,data[1].playername,sourceplayername,duree,reason,0,json.decode(data[1].hwid)) --Timed ban here
							else --Here if perm ban
								ban(source,data[1].license,data[1].identifier,data[1].liveid,data[1].xblid,data[1].discord,data[1].playerip,data[1].playername,sourceplayername,duree,reason,1,json.decode(data[1].hwid)) --Perm ban here
							end
						else
							TriggerEvent('bansql:sendMessage', source, Text.invalidtime)
						end
...
function ban(source,license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,duree,reason,permanent,hwid)
	MySQL.Async.fetchAll('SELECT * FROM banlist WHERE license = @playername', 
	{
		['@playername'] = license
	}, function(data)
		if not data[1] then
			...
			MySQL.Async.execute(
					'INSERT INTO banlist (license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,reason,expiration,timeat,permanent,hwid) VALUES (@license,@identifier,@liveid,@xblid,@discord,@playerip,@targetplayername,@sourceplayername,@reason,@expiration,@timeat,@permanent,@hwid)',
					{ 
					['@license']          = license,
					['@identifier']       = identifier,
					['@liveid']           = liveid,
			...
			MySQL.Async.execute(
					'INSERT INTO banlisthistory (license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,reason,added,expiration,timeat,permanent,hwid) VALUES (@license,@identifier,@liveid,@xblid,@discord,@playerip,@targetplayername,@sourceplayername,@reason,@added,@expiration,@timeat,@permanent,@hwid)',
					{ 
					['@license']          = license,
					['@identifier']       = identifier,
					['@liveid']           = liveid,
...
function deletebanned(license) 
	MySQL.Async.execute(
		'DELETE FROM banlist WHERE identifier=@license',
		{
		  ['@license']  = license
		},
		function ()
...
function checkHWID(hwidBefore, hwidAfter)
    if hwidBefore ~= nil then
        -- hwidBefore = json.decode(hwidBefore)
        for k,v in pairs(hwidAfter) do
            for k1,v1 in pairs(json.decode(hwidBefore)) do
