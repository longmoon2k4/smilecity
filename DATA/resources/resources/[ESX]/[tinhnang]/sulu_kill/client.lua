...
AddEventHandler('esx:onPlayerDeath', function(data)
    local playerPed = PlayerPedId()
    killer = GetPedSourceOfDeath(playerPed)
    killername = GetPlayerName(data.killerClientId)
    _,weapon = GetCurrentPedWeapon(killer,1)
    killerweapon = reverseWeaponHash( tostring(weapon) )
    ...
    if killer == playerPed then
        TriggerServerEvent('playerDied',0,0)
    elseif killername then
        TriggerServerEvent('playerDied',killername,1,killerweapon,data.killerServerId)
    else
        TriggerServerEvent('playerDied',killername,3,killername,data.killerServerId)
    end
end)
