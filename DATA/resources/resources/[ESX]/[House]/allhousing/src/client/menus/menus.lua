  ...
  if Config.UsingESX then
    if not Config.UsingDiscInventory then
      ESX.TriggerServerCallback('Allhousing:GetInventory', function(inventory)
        --TriggerEvent("esx_inventoryhud:openPropertyInventory", inventory, InsideHouse.Id, false, InsideHouse)
        --exports.ox_inventory:openInventory('house', {id='house'..InsideHouse.Id,name='house'..InsideHouse.Id,weight=inventory.cannangkhodo})
        exports.ox_inventory:openInventory('stash', {id='house'..InsideHouse.Id, owner=InsideHouse.Owned})
      end,InsideHouse)  
    else
