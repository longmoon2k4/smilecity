...
RegisterKeyMapping('khoaxe', 'Khoa xe', 'keyboard', "u")
    RegisterCommand('khoaxe', function()
		...
		notowned = 0
		if #cars == 0 then
			ESX.ShowNotification("Không có xe nào gần bạn.")
		else
			for j=1, #cars, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							ESX.ShowNotification("Bạn đã khóa xe")
							--exports['jnr_notify']:Alert("Hệ Thống", "Bạn đã khóa xe", 5000, 'error')
						...
						elseif lock == 2 then
							...
							ESX.ShowNotification("Bạn đã mở khóa xe")
							--exports['jnr_notify']:Alert("Hệ Thống", "Bạn đã mở khóa xe", 5000, 'success')
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							hasAlreadyLocked = true
					...
					if notowned == #carstrie then
						ESX.ShowNotification("~r~Không có xe nào ở gần")
						--exports['jnr_notify']:Alert("Hệ Thống", "Không có xe nào ở gần", 5000, 'error')
					end	
				end, plate)
			end			
		end
