...
local function raycast(flag)
	...
	local rayHandle = StartShapeTestCapsule(playerCoords.x, playerCoords.y, playerCoords.z + 0.5, plyOffset.x, plyOffset.y, plyOffset.z, 10.0, flag or 30, cache.ped)
	while true do
		Wait(0)
...
local function startFueling(vehicle, isPump)
	...
	local duration = math.floor((100 - fuel) / Config.refillValue) * Config.refillTick + 3000
	...
	if isPump then
		...
		if Config.priceTick > moneyAmount then
			return lib.notify({
				type = 'error',
				description = locale('not_enough_money', Config.priceTick), position = 'bottom'
			})
		end
	elseif Config.durabilityTick > fuelingCan.metadata.ammo then
		return lib.notify({
			type = 'error',
			description = locale('petrolcan_not_enough_fuel'), position = 'bottom'
		})
	end
	...
	RegisterCommand('startfueling', function()
		...
		if nearestPump then
			...
			if not vehicleInRange then
				...
				return lib.notify({type = 'error', description = locale('petrolcan_cannot_afford'), position = 'bottom'})
			elseif moneyAmount >= Config.priceTick then
				return startFueling(lastVehicle, true)
			else
				return lib.notify({type = 'error', description = locale('refuel_cannot_afford'), position = 'bottom'})
			end

			return lib.notify({type = 'error', description = locale('vehicle_far'), position = 'bottom'})
			...
			options = {
				{
					action = function (entity)
						if ox_inventory:Search(2, 'money') >= Config.priceTick then
							...
							lib.notify({type = 'error', description = locale('refuel_cannot_afford'), position = 'bottom'})
						end
					end,
					...
					action = function (entity)
						if ox_inventory:Search(2, 'money') >= Config.priceTick then
							if GetVehicleFuelLevel(lastVehicle) >= 100 then
								return lib.notify({type = 'error', description = locale('vehicle_full'), position = 'bottom'})
							end
							startFueling(lastVehicle, 1)
						else
							lib.notify({type = 'error', description = locale('refuel_cannot_afford'), position = 'bottom'})
			...
			options = {
				{
					action = function (entity)
						...
						if not petrolCan then return lib.notify({type = 'error', description = locale('petrolcan_not_equipped'), position = 'bottom'}) end
...
AddEventHandler('doxang',function()
	local vehicle = raycast()
	local petrolCan = GetSelectedPedWeapon(cache.ped) == `WEAPON_PETROLCAN`
	if not petrolCan then return lib.notify({type = 'error', description = locale('petrolcan_not_equipped'), position = 'bottom'}) end
	if fuelingCan.metadata.ammo <= Config.durabilityTick then return end
...
local function getPetrolCan2(refuel)
	TaskTurnPedToFaceCoord(cache.ped, GetEntityCoords(cache.ped), Config.petrolCan.duration)
	Wait(500)
...
RegisterCommand('doxang', function()
