...
RegisterServerEvent('esx_holdup:tooFarBank')
AddEventHandler('esx_holdup:tooFarBank', function(currentStore)
	local _source = source
	rob = false
	-- local xPlayers = ESX.GetExtendedPlayers() -- Returns all xPlayers
	-- for _, xPlayer in pairs(xPlayers) do
		TriggerClientEvent('esx:showNotification', -1, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
		TriggerClientEvent('esx_holdup:killBlipBank', -1)
   --end

	if robbers[_source] then
		TriggerClientEvent('esx_holdup:tooFarBank', _source)
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
	end
end)

RegisterServerEvent('esx_holdup:robberyStartedBank')
AddEventHandler('esx_holdup:robberyStartedBank', function(currentStore)
	local _source  = source
	local xPlayer  = ESX.GetPlayerFromId(_source)
	--local xPlayers = ESX.GetExtendedPlayers()
	if Stores[currentStore].team.total >= 6 and Stores[currentStore].team.total <= 15 then
			...
			if not rob then
				...
				if cop >= Config.PoliceNumberRequired then
					if lastrob <= os.time() then
						...
						rob = true
						--for _, xPlayer in pairs(xPlayers) do
							TriggerClientEvent('esx:showNotification',-1, _U('rob_in_prog', store.nameOfStore, Stores[currentStore].team.total))
							TriggerClientEvent('esx_holdup:setBlipBank', -1, Stores[currentStore].position)
						--end
						...
						TriggerClientEvent('esx_holdup:currentlyRobbingBank', _source, currentStore)
						TriggerClientEvent('esx_holdup:startTimerBank', _source)

						for k, v in pairs(Stores[currentStore].team.members) do 
							TriggerEvent('PlayersName:SetTruyNa', v.playerId, 20,5)
						end
						...
						SetTimeout(store.secondsRemaining * 1000, function()
							if robbers[_source] then
								rob = false
								if xPlayer then
									TriggerClientEvent('esx_holdup:robberyCompleteBank', _source, store.reward)
									lastrob = os.time() + 2500
		...
		TriggerClientEvent('esx:showNotification', _source, 'Chưa đủ 6 người để cướp')
	end
end)

RegisterServerEvent('esx_holdup:jointeamBank')
AddEventHandler('esx_holdup:jointeamBank', function(teamName)
	local _source  = source
	--local xPlayer  = ESX.GetPlayerFromId(_source)
	...
	if Stores[teamName].team.total < 15 then
	    ...
	    TriggerClientEvent('esx_holdup:recieveconfigBank', -1, Stores)
	    TriggerClientEvent('esx_holdup:joinedteamBank', _source, true)
	    TriggerClientEvent('esx_holdup:prepteamBank', _source, true)
...
RegisterServerEvent('esx_holdup:leaveteamBank')
AddEventHandler('esx_holdup:leaveteamBank', function(teamName)
	...
	--local xPlayer = ESX.GetPlayerFromId(_source)
		...
		for k, v in pairs(Stores[teamName].team.members) do 
			...
			if v.playerId == _source then
				...
				TriggerClientEvent('esx_holdup:joinedteamBank', _source, false)
